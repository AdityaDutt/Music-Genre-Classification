...
# Find total files
for root, dirs, files in os.walk("/Users/richadutt/Documents/ranga/cap6610sp19_project/Validation_Set/Prog", topdown=False):
   for name in files:
...
for root, dirs, files in os.walk("/Users/richadutt/Documents/ranga/cap6610sp19_project/Validation_Set/NonProg", topdown=False):
   for name in files:
      filename = os.path.join(root, name)
      if filename.find(".mp3") != -1 :
          total_files = total_files + 1
...
data = pd.read_csv('validation_features.csv')
data.head()            
